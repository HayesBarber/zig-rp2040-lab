...
Context switching is not implemented yet, so this initial implemenation will just be a super loop of tasks. When we get to that point we will need to push an initial stack frame onto each tasks stack and set its stack pointer accordinly.
